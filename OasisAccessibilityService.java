...
public class OasisAccessibilityService extends AccessibilityService {
    ...
    private static final long MENU_COOLDOWN_MS = 650;
    ...
    // OCR é criado somente quando realmente necessário.
    private TextRecognizer recognizer = null;

    // IDs lógicos 1..5. As coordenadas são sempre atualizadas.
    ...
    private final Runnable loop = new Runnable() {
        @Override public void run() {
            ...
            if (OASIS_PACKAGE.equals(lastPackage)
    ...
    protected void onServiceConnected() {
        ...
        Log.i(TAG, "Oasis Autoplay v0.3.1 iniciado");
    }
    ...
    private void analyzeScreen() {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R) {
            Log.w(TAG, "takeScreenshot requer Android 11+");
            return;
        }
        ...
        try {
                    ...
                    new TakeScreenshotCallback() {
                        ...
                        public void onSuccess(ScreenshotResult result) {
                            ...
                            try {
                                ...
                                // Não derruba o AccessibilityService.
            ...
            // Proteção adicional contra crash de fabricante/Android.
            screenshotBusy = false;
    ...
    private void detectStateAndAct(Bitmap b) {
        ...
        if (purple > 0.30) {
            ...
            if (!attackBuffUsed) {
                // Seleciona Attack uma única vez.
                ...
                if (tap(attackIconX, attackIconY)) {
                    ...
                    // Confirma depois que a UI teve tempo para atualizar.
                    main.postDelayed(() -> {
                        if (destroyed) return;
        ...
        // 3. Menu de ataque no mapa.
        double magenta =
                ratio(b, 0.02, 0.69, 0.49, 0.82, 16, PixelKind.MAGENTA);

        if (magenta > 0.10 && !inCombat) {
            performAction("MAP_ATTACK",
                    0.25f * w, 0.765f * h, MENU_COOLDOWN_MS);
            return;
        }

        // Evita decidir enquanto a interface ainda está reagindo ao clique.
        ...
        if (candidates.size() >= 2) {
            ...
            // Se chegamos à limpeza 4/5, OCR apenas nesse momento.
    ...
    private void readFourFiveCountsAndAttack(
        ...
        try {
                    ...
                    .addOnSuccessListener(text4 -> {
                        ...
                        recognizer.process(InputImage.fromBitmap(crop5, 0))
                                .addOnSuccessListener(text5 -> {
                                    ...
                                    if (n4 >= 0 && n5 >= 0) {
                                        target = n4 >= n5 ? t4 : t5;
                                        Log.i(TAG, "OCR 4=" + n4 + " 5=" + n5
                                                + " -> Troll " + target.id);
                                    } else {
                                        // Fallback: o mais avançado/perigoso.
                                        target = t4.y >= t5.y ? t4 : t5;
                                        Log.w(TAG, "OCR parcial; fallback Troll "
                                                + target.id);
                                    }
            ...
            Log.e(TAG, "OCR indisponível; usando fallback", t);
            StackTrack target = t4.y >= t5.y ? t4 : t5;
            attackTrack(target, w, h);
        }
    }
    ...
    // Procura pequenas caixas escuras dos números e usa sua posição
    // para estimar o centro atual do sprite.
    ...
    private void updateTracks(List<StackCandidate> cands) {
        if (tracks.isEmpty()) {
            ...
            // Os cinco candidatos mais coerentes da formação inicial.
            for (int i = 0; i < 5; i++) {
            ...
            Log.i(TAG, "Trolls 1..5 identificados");
            return;
        }
        ...
        for (int id = 1; id <= 5; id++) {
            ...
            // Troll pode andar, então acompanhamos o sprite,
            // mas nunca atacamos a coordenada inicial fixa.
            if (best != null && bestDistance < 230) {
                ...
                // Só considera morto/desaparecido após várias leituras.
                if (t.missed >= 3) {
                    t.alive = false;
                    Log.i(TAG, "Troll " + id + " não detectado; marcado fora");
                }
            }
        }
    }
    ...
    private void chooseStrategicTargetAndTap(int w, int h) {
        ...
        // Abertura: atacar o Troll 2 explora o splash em 1,2,3.
        StackTrack t2 = tracks.get(2);
        if (!openingSplashDone && t2 != null && t2.alive) {
            target = t2;
            openingSplashDone = true;
            Log.i(TAG, "ALVO abertura: Troll 2");
        }

        // Depois, qualquer sobrevivente da frente 1-3 ganha prioridade.
        if (target == null) {
            ...
            if (urgent != null) {
                target = urgent;
                Log.i(TAG, "ALVO frente: Troll " + target.id);
            }
        }

        // Se sobrou apenas um entre 4 e 5, ataca esse.
        if (target == null) {
            StackTrack t4 = tracks.get(4);
            StackTrack t5 = tracks.get(5);
        ...
        // Fallback geral: o Troll mais avançado.
        if (target == null) {
