...
public class OasisAccessibilityService extends AccessibilityService {
    ...
    public static final String PREF_ONLINE_AI_ENABLED = "online_ai_enabled";
    public static final String PREF_ONLINE_API_KEY = "online_api_key";
    public static final String PREF_ONLINE_AI_STATUS = "online_ai_status";
    public static final String PREF_ONLINE_AI_LAST_SCENE = "online_ai_last_scene";
    public static final String PREF_ONLINE_AI_LAST_ACTION = "online_ai_last_action";
    public static final String PREF_ONLINE_AI_LAST_ERROR = "online_ai_last_error";
    ...
    private volatile boolean onlineBusy = false;
    private long lastOnlineRequestMs = 0L;
    private static final long ONLINE_MIN_INTERVAL_MS = 3500L;
    private static final long ONLINE_STALL_TRIGGER_MS = 2600L;
    private OnlineVisionClient onlineVisionClient;
    ...
    protected void onServiceConnected() {
        ...
        onlineVisionClient = new OnlineVisionClient();
        loadTrollReference();
        loadSceneReferences();
        loadSpawnMemory();
        ...
        Log.i(TAG, "Oasis Autoplay screen-first robusto iniciado");
    }
    ...
    public void onDestroy() {
        ...
        try {
            if (onlineVisionClient != null) onlineVisionClient.shutdown();
        } catch (Throwable ignored) {}
    ...
    private void detectAndAct(Bitmap b) {
        ...
        // -----------------------------------------------------------------
        // MOTOR SCREEN-FIRST
        // A captura atual manda. EngineState serve como memória curta e nunca
        // pode impedir uma ação que a própria tela comprova ser válida.
        // -----------------------------------------------------------------
        final VisualScene visualScene = classifyVisualScene(b);
        saveVisualScene(visualScene, lastVisualConfidence);

        final boolean victoryObserved =
                visualScene == VisualScene.VICTORY || detectVictory(b);
        final boolean levelUpObserved = detectLevelUpScreen(b);
        final boolean magicObserved =
                visualScene == VisualScene.MAGIC || detectMagicScreen(b);
        final boolean turnMenuObserved = !magicObserved &&
                (visualScene == VisualScene.TURN_MENU || detectTurnMenu(b));
        final boolean spellTargetObserved = !magicObserved && !turnMenuObserved
                && detectSpellTargetMode(b);

        // Detecta stacks uma única vez por captura. Além de economizar CPU,
        // isso permite recuperar uma batalha mesmo quando a IA visual retorna UNKNOWN.
        final List<StackCandidate> stacks = findStackCandidates(b);
        final boolean battlefieldObserved =
                visualScene == VisualScene.BATTLEFIELD
                || visualScene == VisualScene.BATTLEFIELD_AFTER_ACTION
                || stacks.size() >= 2;

        // O menu do Troll tem prioridade sobre qualquer estado velho, mas é
        // mascarado quando existe evidência mais forte de interface de combate.
        final boolean targetMenuObserved =
                !victoryObserved
                && !magicObserved
                && !turnMenuObserved
                && !spellTargetObserved
                && (visualScene == VisualScene.TARGET_MENU || detectTargetMenu(b));

        // IA online híbrida: só entra quando a leitura local está incerta ou
        // o motor está sem progresso. Nunca substitui uma tela local clara.
        final boolean strongLocalObservation = victoryObserved
                || levelUpObserved
                || targetMenuObserved
                || magicObserved
                || turnMenuObserved
                || spellTargetObserved
                || battlefieldObserved
                || visualScene == VisualScene.MAP;

        if (!strongLocalObservation
                && shouldAskOnlineVision(now, visualScene)) {
            requestOnlineVision(b, now);
            return;
        }

        // Expectativas antigas são apenas diagnósticas. Se a tela conhecida divergir,
        // ressincronizamos imediatamente e continuamos processando a MESMA captura.
        if (expectedVisualScene != VisualScene.UNKNOWN
                && visualScene != VisualScene.UNKNOWN
                && !isCompatibleFlowScene(expectedVisualScene, visualScene)) {
            VisualScene oldExpected = expectedVisualScene;
            expectedVisualScene = VisualScene.UNKNOWN;
            flowFailureCount = 0;
            saveFlowExpected(expectedVisualScene);
            saveFlowFailures(0);
            setVisibleStateOnly(
                    "Ressincronizado: " + oldExpected.name()
                    + " -> " + visualScene.name()
            );
        }

        // 1) VITÓRIA: prioridade máxima.
        if (victoryObserved) {
            expectedVisualScene = VisualScene.UNKNOWN;
            flowFailureCount = 0;
            saveFlowExpected(expectedVisualScene);
            ...
            setState(EngineState.VICTORY, "Vitória reconhecida");
            lastVictoryMs = now;
            ...
            if (performAction(
                ...
                setState(EngineState.ROUTE_MOVE,
        ...
        // 3) MENU DO TROLL. Não depende de combatContext ou estado anterior.
        // Se o usuário abrir o Troll manualmente, o bot assume daqui.
        if (targetMenuObserved) {
            expectedVisualScene = VisualScene.UNKNOWN;
            flowFailureCount = 0;
            saveFlowExpected(expectedVisualScene);
            saveFlowFailures(0);

            // Estado de combate antigo não pode sequestrar este menu.
            if (isCombatState(state)) {
                resetBattleTracking();
            }

            setState(EngineState.TARGET_MENU, "Menu do Troll confirmado");
            ...
            if (performAction(
                ...
                setState(EngineState.ENTERING_COMBAT,
                        "Atacar pressionado; aguardando batalha");
            }
            return;
        }

        // Recompensa pós-luta vem depois do menu de alvo para não roubar telas.
        if (detectReward(b)) {
        ...
        // 4) TELA DE MAGIAS. A própria tela estabelece contexto de combate.
        if (magicObserved && !attackBuffUsed) {
            setState(EngineState.CAST_ATTACK_BUFF,
                    magicSpellSelected
                            ? "Bênção encontrada; confirmar Selecionar"
                            : "Procurando Bênção pelo efeito de ataque");
            castAttackBuff(b);
            return;
        }

        // 5) MODO DE ALVO DA MAGIA. Se esta tela existe, então a seleção da
        // magia já aconteceu mesmo que algum estado anterior tenha sido perdido.
        if (spellTargetObserved && !attackBuffUsed) {
            magicSpellSelected = true;
            setState(EngineState.APPLY_ATTACK_TARGET,
                    attackTargetTapped
                            ? "Bênção aplicada; aguardando campo"
                            : "Aplicando Bênção na própria tropa");

            if (!attackTargetTapped) {
                float[] own = findOwnUnitTarget(b);
                float tx = own != null ? own[0] : 0.32f * b.getWidth();
                float ty = own != null ? own[1] : 0.55f * b.getHeight();
        ...
        // 6) MENU DE TURNO. Também independe do estado antigo.
        if (turnMenuObserved && !attackBuffUsed) {
        ...
        // 7) CAMPO DE BATALHA. Recupera o combate mesmo a partir de UNKNOWN.
        if (battlefieldObserved) {
            if (!isCombatState(state)) {
                setState(EngineState.BATTLEFIELD,
                        "Batalha recuperada pela tela atual");
            }
            ...
            // Tocamos na própria tropa e o modo de alvo desapareceu:
            // esta é a confirmação operacional do único buff do combate.
            if (!attackBuffUsed
                    && magicSpellSelected
                    && attackTargetTapped) {
                attackBuffUsed = true;
                ...
                saveFlowFailures(0);
                setState(EngineState.BATTLEFIELD,
                        "Bênção confirmada; iniciar ataques");
                nextAllowedAnalysisMs = now + 650;
                return;
            }

            // Às vezes o modo de alvo é visualmente parecido com o campo e o
            // detector específico falha. Após uma pequena espera, ainda aplicamos
            // na tropa própria em vez de reabrir menus e perder a magia selecionada.
            if (!attackBuffUsed
                    && magicSpellSelected
                    && !attackTargetTapped
                    && state == EngineState.APPLY_ATTACK_TARGET
                    && now - magicPhaseSinceMs > 700) {
                float[] own = findOwnUnitTarget(b);
                float tx = own != null ? own[0] : 0.32f * b.getWidth();
                float ty = own != null ? own[1] : 0.55f * b.getHeight();
                if (performAction(
                        "4_APLICAR_BENCAO_RECUPERACAO",
                        tx, ty, 1000)) {
                    attackTargetTapped = true;
                    magicPhaseSinceMs = now;
                }
            ...
            // Antes de atacar, o único objetivo é abrir Ação -> Usar magia.
            if (!attackBuffUsed) {
                setState(EngineState.BATTLEFIELD,
                        "Batalha: preparando Bênção");
                if (performAction(
                        "0_ABRIR_MENU_ACAO",
                        0.25f * b.getWidth(),
                        0.91f * b.getHeight(),
        ...
        // 8) MAPA / ROTA. Só chega aqui quando nenhuma interface mais importante
        // foi reconhecida. Portanto a rota não pode passar por cima de menu/magia.
        ...
        if (state == EngineState.TARGET_PENDING) {
            setState(EngineState.SEARCH_MAP,
                    "Alvo rejeitado; procurando outro");
        ...
        if (state == EngineState.SEARCH_MAP
                || state == EngineState.RECOVER
                || visualScene == VisualScene.MAP
                || visualScene == VisualScene.UNKNOWN) {
        ...
        // Segurança final: nunca mantém estado intermediário indefinidamente.
        if (now - lastProgressMs > 4500) {
            expectedVisualScene = VisualScene.UNKNOWN;
            flowFailureCount = 0;
            saveFlowExpected(expectedVisualScene);
            saveFlowFailures(0);
            waitingForScreenChange = false;
            tracks.clear();
            setLastError("Ressincronização automática em " + state.name());
            setState(EngineState.RECOVER, "Recuperando pela tela atual");
            nextAllowedAnalysisMs = now + 600;
        }
    }

    private boolean shouldAskOnlineVision(long now, VisualScene visualScene) {
        SharedPreferences p = getSharedPreferences(PREFS, MODE_PRIVATE);
        if (!p.getBoolean(PREF_ONLINE_AI_ENABLED, false)) return false;
        String key = p.getString(PREF_ONLINE_API_KEY, "");
        if (key == null || key.trim().isEmpty()) {
            p.edit().putString(PREF_ONLINE_AI_STATUS, "Ativada, mas sem chave API").apply();
            return false;
        }
        if (onlineBusy) return false;
        if (now - lastOnlineRequestMs < ONLINE_MIN_INTERVAL_MS) return false;
        return visualScene == VisualScene.UNKNOWN
                || lastVisualConfidence < 0.55
                || now - lastProgressMs >= ONLINE_STALL_TRIGGER_MS;
    }

    private void requestOnlineVision(Bitmap screen, long now) {
        if (onlineVisionClient == null || onlineBusy) return;
        SharedPreferences p = getSharedPreferences(PREFS, MODE_PRIVATE);
        String key = p.getString(PREF_ONLINE_API_KEY, "");
        ...
        Bitmap copy;
        try {
            copy = screen.copy(Bitmap.Config.ARGB_8888, false);
        } catch (Throwable t) {
            setOnlineError("Falha ao copiar screenshot: " + shortError(t));
            return;
        }
        if (copy == null) return;
        ...
        p.edit().putString(PREF_ONLINE_AI_STATUS, "Consultando IA online...").apply();

        final int w = screen.getWidth();
        final int h = screen.getHeight();

        onlineVisionClient.analyze(copy, key.trim(), new OnlineVisionClient.Callback() {
            @Override
            public void onSuccess(OnlineVisionClient.Decision d) {
                try { if (!copy.isRecycled()) copy.recycle(); } catch (Throwable ignored) {}
                main.post(() -> {
                    onlineBusy = false;
                    applyOnlineDecision(d, w, h);
                });
            }
            ...
            public void onError(String error) {
                ...
                main.post(() -> {
                    ...
                    nextAllowedAnalysisMs = System.currentTimeMillis() + 700;
                });
            }
        });
    }

    private void applyOnlineDecision(OnlineVisionClient.Decision d, int w, int h) {
        if (d == null || destroyed || !isAutoplayEnabled()) return;
        SharedPreferences p = getSharedPreferences(PREFS, MODE_PRIVATE);
        p.edit()
                .putString(PREF_ONLINE_AI_STATUS,
                        "Online: " + d.scene + " / " + d.action +
                        " conf=" + String.format(java.util.Locale.US, "%.2f", d.confidence))
                .putString(PREF_ONLINE_AI_LAST_SCENE, d.scene)
                .putString(PREF_ONLINE_AI_LAST_ACTION, d.action)
                .putString(PREF_ONLINE_AI_LAST_ERROR, "")
                .apply();
        ...
        String action = d.action == null ? "NOOP" : d.action;
        switch (action) {
            case "ATTACK_TARGET_MENU":
                resetFlowForOnline();
                setState(EngineState.TARGET_MENU, "IA online confirmou menu do Troll");
                if (performAction("ONLINE_ATACAR_ALVO", 0.25f * w, 0.735f * h, MENU_COOLDOWN_MS)) {
                    increment(PREF_TARGET_COUNT);
                    setState(EngineState.ENTERING_COMBAT, "Online: Atacar pressionado");
                }
                break;
            case "OPEN_ACTION_MENU":
                resetFlowForOnline();
                setState(EngineState.BATTLEFIELD, "IA online confirmou batalha");
                performAction("ONLINE_ABRIR_ACAO", 0.25f * w, 0.91f * h, 1000);
                break;
            case "OPEN_USE_MAGIC":
                resetFlowForOnline();
                setState(EngineState.TURN_MENU, "IA online confirmou menu de turno");
                performAction("ONLINE_USAR_MAGIA", 0.74f * w, 0.60f * h, 1000);
                break;
            case "TAP_BLESSING_SLOT": {
                float x = clamp01(d.x);
                float y = clamp01(d.y);
                if (x >= 0.05f && x <= 0.95f && y >= 0.14f && y <= 0.86f) {
                    setState(EngineState.CAST_ATTACK_BUFF, "IA online localizou Bênção");
                    performAction("ONLINE_TOCAR_BENCAO", x * w, y * h, 900);
                }
                break;
            }
            case "APPLY_BLESSING_SELF": {
                float x = clamp01(d.x);
                float y = clamp01(d.y);
                if (x >= 0.10f && x <= 0.58f && y >= 0.25f && y <= 0.82f) {
                    magicSpellSelected = true;
                    setState(EngineState.APPLY_ATTACK_TARGET, "IA online: aplicar Bênção");
                    if (performAction("ONLINE_APLICAR_BENCAO", x * w, y * h, 1000)) {
                        attackTargetTapped = true;
            ...
            case "CLOSE_VICTORY":
                resetFlowForOnline();
                setState(EngineState.VICTORY, "IA online confirmou vitória");
                if (performAction("ONLINE_FECHAR_VITORIA", 0.25f * w, 0.91f * h, VICTORY_COOLDOWN_MS)) {
                    markLastConfirmedSpawnDefeated(System.currentTimeMillis());
                    markCurrentHuntZoneDefeated(System.currentTimeMillis());
                    resetBattleTracking();
                }
                break;
            case "TAP_TROLL": {
                float x = clamp01(d.x);
                float y = clamp01(d.y);
                if (x >= 0.05f && x <= 0.95f && y >= 0.12f && y <= 0.80f) {
                    setState(EngineState.TARGET_PENDING, "IA online encontrou Troll");
                    pendingTargetNormX = x;
                    pendingTargetNormY = y;
                    performAction("ONLINE_TOCAR_TROLL", x * w, y * h, MAP_TAP_COOLDOWN_MS);
                }
                break;
            }
            default:
    ...
    private void resetFlowForOnline() {
        expectedVisualScene = VisualScene.UNKNOWN;
        flowFailureCount = 0;
        waitingForScreenChange = false;
        saveFlowExpected(expectedVisualScene);
    ...
    private float clamp01(double v) {
        if (Double.isNaN(v) || Double.isInfinite(v)) return 0.5f;
        return (float)Math.max(0.0, Math.min(1.0, v));
    }

    private void setOnlineError(String error) {
        getSharedPreferences(PREFS, MODE_PRIVATE).edit()
                .putString(PREF_ONLINE_AI_STATUS, "Online indisponível")
                .putString(PREF_ONLINE_AI_LAST_ERROR, error == null ? "erro" : error)
                .apply();
    }
        ...
        if (magicSpellSelected) {
            if (performAction(
                ...
                setState(
                        ...
                        "Bênção selecionada; aguardando alvo"
                );
            }
            return;
        }
